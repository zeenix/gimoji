...
**Status:** Implemented — rendering approach superseded twice; see final-state postscript
**Date:** 2026-06-06
...
## Postscript: final state (2026-08-11)

The consolidation goal — one Rust source of truth for picker behaviour — landed as designed:
`gimoji-core` owns `App`/`Action`/`Outcome`, the widgets, search, palettes, and the `Clipboard`
trait; the native CLI is externally unchanged.

The rendering half of this document is historical. The `ratatui-wgpu` + bundled-font pipeline
shipped first but its WebGL2 surface, glyph atlas, and raster stack kept an idle tab at
50–100 MB resident with a ~3 MB wasm bundle. It was replaced by ratzilla's DOM backend
(see `2026-06-08-dom-only-web-frontend.md`), then by a hand-rolled per-cell DOM backend, and
finally by the Canvas-2D backend in `crates/gimoji-web/src/canvas_backend.rs`. The
"pixel-faithful look across native and web" goal was consciously relaxed along the way: the web
build renders with system monospace + colour-emoji fonts instead of bundled fonts.

Two deliberate deltas against pre-branch native came along for the ride: the selection list's
columns moved from percentage-based to fixed + fill constraints (with `HighlightSpacing::Always`,
so column positions no longer shift as the selection moves), and search matching went from a
Unicode case-insensitive regex to ASCII case folding — both minor, and both visible if you diff
screenshots against the old build.

One more delta, this one against the Clipboard section above: the shipped web frontend waits for
the browser's write promise to settle and toasts what actually happened — "Copied" on success,
"Copy failed" on rejection — instead of the fire-and-forget copy described there.

Final numbers (2026-08-11, measured via the Task 2 smoke harness): 214 KiB raw wasm
(89.4 KiB gzipped), JS heap ~1.76 MiB (`JSHeapUsedSize` in headless Chrome — see the
DOM-frontend spec's Idle RSS section for the measurement caveat and why no Firefox
number was redone). Bundle-size target of ≤ 1 MB comfortably met.
